...
每次被接受的 `send-to-claude`、`send-to-codex` 和 `reply` 都会返回一个投递令牌：`dlv_` 后跟恰好 24 个 base64url 字符。它指向私有 v4 状态中一条有界的消息／状态记录，不是提供方回执句柄。令牌只持久化在 mode-0600 的代理状态中，绝不会进入公开快照、普通日志、提供方回执或任何仪表盘。
