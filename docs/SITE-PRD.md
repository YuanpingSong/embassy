...
- **Protocol truth.** The site's signature section — keep it, and fix its density. Today these are 100-word documentation paragraphs; the requirement is the same truths in scannable form (the reader should be able to *hold* each truth): delivered ≠ read; delivery timing is directional (Codex-bound ordinary work waits honestly for idle/turn boundaries, while Claude-bound work enters Claude's mailbox immediately regardless of observed busy/idle); Claude-bound `transport_written` is that direction's terminal `delivered` mailbox-write boundary, not proof of reading or consumption; restarts drop bodies on purpose; exact-version compatibility is checked automatically and unknown versions fail closed; `embassy serve` opens no network listener (the opt-in live companion is the sole loopback listener); a routed message is a new input path — treat it as untrusted input to its receiver.
- **Quickstart.** Honest requirements first (macOS, Node ≥ 20, exact version pins), then the full command sequence. Only the token-holder can `reply`; the other direction needs no Embassy commands at all (native `ListAgents`/`SendMessage`).
...
- **Compatibility is not an operator workflow.** The site may show passive exact-version status, but must never teach a manual compatibility, scheduler, or override workflow. Startup and replacement endpoint-generation validation are broker-owned and automatic.
- **Never flatten directional scheduling.** No current copy or animation may say
  that Claude-bound work waits for Claude to become idle or queues because
  Claude is busy. Codex-bound idle gating and the exact next-tool-call-boundary
  `STEER:` exception remain explicit.
