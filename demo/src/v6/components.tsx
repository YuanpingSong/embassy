...
export const DispatchCard = ({x, y, width, height, product, handle, provider, children, oneLine = false, style, scrollBody = false, bodyPadding}: {x: number; y: number; width: number; height: number; product: string; handle: string; provider: 'claude' | 'codex' | 'tui'; children: ReactNode; oneLine?: boolean; style?: CSSProperties; scrollBody?: boolean; bodyPadding?: string}) => <section style={{position: 'absolute', left: x, top: y, width, height, boxSizing: 'border-box', background: p.panel, border: `1px solid ${p.lineDark}`, borderRadius: 8, overflow: 'hidden', ...style}}>
