  ...
  try {
    ...
    if (object(result) && Array.isArray(result.warnings)) {
      for (const warning of result.warnings.filter(object)) {
        ...
        const detail = warning.reason === "stale_older"
          ? `using newest PID ${warning.selectedPid}; older PID(s) ${otherPids.join(", ")}. Exit the older Claude process, then run embassy refresh`
          : warning.reason === "unreachable_newest"
            ? `using PID ${warning.selectedPid}; newest PID ${warning.newestPid} was not confirmed reachable. Check it before exiting anything, then run embassy refresh`
            : `none of the duplicate sockets for PID(s) ${[warning.selectedPid, ...otherPids].join(", ")} was confirmed reachable. Check the Claude processes before exiting anything, then run embassy refresh`;
        stderr.write(`[embassy] ${warning.code}: ${warning.alias}; ${detail}. Do not retire your own route to fix delivery.\n`);
      }
    }
