import { getDashboardCopy, type DashboardLocale } from "./dashboard-copy.js";
...
const LIVE_COPY = Object.freeze({
  en: Object.freeze({
    ...
    connected: "Live",
    unavailable: "Observer unavailable — retrying",
    disconnected: "Connection ended — use Reconnect",
    fatal: "Dashboard session unavailable",
    paused: "Updates paused",
    stopped: "Dashboard stopped",
    pause: "Pause",
    reconnect: "Reconnect",
    refresh: "Read now",
    filter: "Filter aliases and activity",
    queued: "Queued",
    active: "Active",
    noAttention: "No active attention items.",
    noActivity: "No delivery metadata yet.",
    sessions: "Sessions",
    diagnostics: "Diagnostics",
    revision: "Stream revision",
    reset: "Source restarted; view resynchronized.",
  }),
  "zh-CN": Object.freeze({
    live: "实时伴侣",
    connecting: "正在连接…",
    connected: "实时",
    unavailable: "观察器不可用 — 正在重试",
    disconnected: "连接已断开 — 请点击“重新连接”",
    fatal: "面板会话不可用",
    paused: "更新已暂停",
    stopped: "面板已停止",
    pause: "暂停",
...
function shell(locale: DashboardLocale): string {
  return `<!doctype html>
<html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>Embassy — live dashboard</title><link rel="stylesheet" href="app.css"><script type="module" src="client.js"></script></head><body><main id="embassy-live" aria-live="polite"></main><noscript>This read-only live dashboard requires local JavaScript. The static Embassy dashboard remains available without JavaScript.</noscript></body></html>`;
}
...
const LIVE=${JSON.stringify(LIVE_COPY)};
...
const l=(key)=>LIVE[locale][key]||key;
...
function setConnectionState(state){connectionState=state;const node=document.getElementById("live-status");if(node){node.textContent=l(state);node.dataset.tone=connectionTone();}}
...
function render(){if(!latest)return;const model=latest.model;root.replaceChildren();const header=el("header","masthead");const title=el("div");append(title,el("p","eyebrow",t("brand.eyebrow")),el("h1","",t("brand.title")),el("p","lede",t("brand.subtitle")));const controls=el("div","controls");const language=el("select","select");language.setAttribute("aria-label",t("language.label"));for(const code of ["en","zh-CN"]){const option=el("option","",code==="en"?t("language.en"):t("language.zhCn"));option.value=code;option.selected=code===locale;language.append(option);}language.addEventListener("change",()=>{locale=language.value;document.documentElement.lang=locale;render();});const search=el("input","filter");search.type="search";search.placeholder=l("filter");search.value=filter;search.addEventListener("input",()=>{filter=search.value.toLowerCase();render();const next=document.querySelector(".filter");if(next){next.focus();next.setSelectionRange(next.value.length,next.value.length);}});append(controls,language,search,button(l("pause"),()=>{if(controller)controller.abort();setConnectionState("paused");}),button(l("reconnect"),connect),button(l("refresh"),readNow));append(header,title,controls);root.append(header);
const status=el("p","live-status",l(connectionState));status.id="live-status";status.dataset.tone=connectionTone();root.append(status);if(latest.reset)root.append(el("p","notice",l("reset")));
