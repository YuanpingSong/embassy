...
- `embassy status` renders for a person when stdout is a terminal: one broker line (pid, version, state directory), one plain word per connector that matters, a routes table with queue depth and last-seen age, the last messages with a one-line body preview, and only those alerts that have a remedy — each non-`ok` word paired with its safe code and one line of what to do about it. Piped, or with `--json`, it prints the snapshot byte-for-byte as before, so every script and the skill are unaffected. `--recent <n>` (1–100, default 10) sizes the message list, colour appears only on a terminal with `NO_COLOR` unset and never carries meaning alone, and `status` rescans for Claude sessions before it reads. Two derivations are deliberate: a connector with nothing to observe is `stale`, not `degraded`, and a shell peer holding unclaimed mail is reported without moving the overall word (emb-107).
- `embassy watch` tails the broker: each new message row once, each settlement once (`accepted → delivered (61 ms)`), and route installs, retirements and discovery refreshes as a secondary line. `--json` streams the same events as JSONL; Ctrl-C exits 0; memory is bounded by the snapshot itself (emb-107).
- `embassy check` is the upstream-drift tripwire to run after a Claude Code or Codex CLI update. It registers a throwaway `peer-*` principal of its own — so it works from a plain terminal with no inherited identity — sends one `[embassy check <id>]` body through the ordinary send path, waits for `delivered`, awaits the correlated reply on its own mailbox, releases the registration, and prints every hop with its timing. `--to <alias>` picks a target instead of the first registered Codex task; `--timeout <s>` (1–600, default 60) bounds each wait; any failing hop exits non-zero with the safe code that explains it. The shipped skill gains the matching rule: a body starting `[embassy check` is answered with one line echoing the id (emb-107).
