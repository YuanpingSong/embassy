...
- v1 advertises exactly **one** registered Codex task per gateway process, and once registered it is visible to every live Claude session running as the same OS user — register only when comfortable with every currently running Claude session, and `unregister-codex` when done.
...
- Inbound Claude reachability does not select that session for outbound Codex-to-Claude delivery. Claude's native `crossSessionInbound` control remains in force.
